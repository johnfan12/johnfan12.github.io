const data = `
  ...
  Z3204040.02	嵌入式系统及应用	期末考试	2024-01-01	19:00~20:35	WT1101*	72	正常
`;

const mannual_add = [['考研', '2022-12-25', '19:00'],
                  ];
...
function initCalendar() {
  let count = 0;
  const events = getEventList(data,mannual_add);
  // 排序
  events.sort(compare);
  // 遍歷事件列表
...
function getEventList(data, mannual_add) {
  ...
  for (const event of mannual_add) {
    const title = `${event[0]}`;
    const time = event[1] + ' ' + event[2];
    events.push({ title, time });
  }
  return events;
}
